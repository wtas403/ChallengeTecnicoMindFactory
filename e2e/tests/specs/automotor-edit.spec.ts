...
test.describe('Automotores - edicion', () => {
  test('actualiza datos y valida guard de cambios pendientes', async ({ page, request }) => {
    ...
    try {
      ...
      await formPage.gotoEdit(draft.dominio);
      await expect(page.locator('#automotor-form-color')).toHaveValue('Plateado E2E');
      await expect(page.locator('#automotor-form-cuit-titular')).toHaveValue(reassignmentCuit);
